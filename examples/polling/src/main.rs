...
struct App {
    offset_multiplayer: f32,
    offset: f32,
    vao: gl::types::GLuint,
    vbo: gl::types::GLuint,
    uniform_offset: gl::types::GLint,
...
impl exposed::window::Event for App {
    fn create(event_handler: &'static mut exposed::window::EventHandler<Self>) -> Option<Self> {
        ...
        let vertex_source = r#"
            ...
            void main() {
                fColor = vColor;
                gl_Position =  vec4(vPosition.x + offset, vPosition.y, 0.0, 1.0);
            }
        "#;
        ...
        let uniform_offset = unsafe { gl::GetUniformLocation(program, "offset\0".as_ptr().cast()) };
    ...
    fn render(&mut self) {
        unsafe {
            ...
            self.offset += 0.01 * self.offset_multiplayer;

            gl::Uniform1f(self.uniform_offset, self.offset);
