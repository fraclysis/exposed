...
pub fn record_submit_commandbuffer<F: FnOnce(&Device, vk::CommandBuffer)>(
    device: &Device, command_buffer: vk::CommandBuffer, command_buffer_reuse_fence: vk::Fence,
    submit_queue: vk::Queue, wait_mask: &[vk::PipelineStageFlags],
    wait_semaphores: &[vk::Semaphore], signal_semaphores: &[vk::Semaphore], f: F,
) {
    unsafe {
        device
...
pub fn find_memorytype_index(
    memory_req: &vk::MemoryRequirements, memory_prop: &vk::PhysicalDeviceMemoryProperties,
...
impl ExampleBase {
    pub fn new(
        ...
        unsafe {
            ...
            //     let a =
            //         Entry::load().unwrap().enumerate_instance_extension_properties(None).unwrap();
            //     for e in a {
            //         log_info!("VkE", "  %s\n", e.extension_name.as_ptr());
            //     }

            //     let a = Entry::load().unwrap().enumerate_instance_layer_properties().unwrap();
            //     for e in a {
            //         log_info!("VkE", "  %s\n", e.layer_name.as_ptr());

            //         let a = Entry::load()
            //             .unwrap()
            //             .enumerate_instance_extension_properties(Some(std::ffi::CStr::from_ptr(e.layer_name.as_ptr())))
            //             .unwrap();
            //         for e in a {
            //             log_info!("VkE", "  %s\n", e.extension_name.as_ptr());
            //         }
            ...
            let surface_format = surface_loader
                .get_physical_device_surface_formats(pdevice, surface______________)
                .unwrap()[0];

            let surface_capabilities = surface_loader
                .get_physical_device_surface_capabilities(pdevice, surface______________)
                .unwrap();
            ...
            let surface_resolution = match surface_capabilities.current_extent.width {
                std::u32::MAX => vk::Extent2D { width: s.width as _, height: s.height as _ },
            ...
            let present_modes = surface_loader
                .get_physical_device_surface_present_modes(pdevice, surface______________)
                .unwrap();
